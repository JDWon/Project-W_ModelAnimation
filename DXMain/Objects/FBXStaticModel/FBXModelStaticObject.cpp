...
#include "FBXModelStaticObject.h"

void CFBXStaticObject::Update(float fTimeElapsed)
{
...
void CFBXStaticObject::Rotate(float radians)
{
	// 업데이트된 모델 매트릭스를 셰이더에 전달하도록 준비합니다.
	m_xmmtxLocal = m_xmmtxLocal * XMMatrixTranspose(XMMatrixRotationY(radians));
}

void CFBXStaticObjectComponent::AddObject(XMVECTOR xmvec, ID3D11Device * pd3dDevice)
{
	unique_ptr<BaseObject> uniqueObj = make_unique<CFBXStaticObject>();
	auto pObj = reinterpret_cast<StaticObject*>(uniqueObj.get());
...
void CFBXStaticObjectComponent::BuildComponent(ID3D11Device * pd3dDevice, ID2D1Factory * pd2dFactory, IDWriteFactory * pdwFactory, IWICImagingFactory * pwicFactory)
{
...
void CFBXStaticObjectComponent::Render(ID3D11DeviceContext * pd3dDeviceContext, CCamera * pCamera)
{
	// Shader Set
...
void CFBXStaticObjectComponent::Update(float fTimeElapsed)
{
